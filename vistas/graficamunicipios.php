				...
				chart = new Highcharts.Chart({
					...
					xAxis: {
						categories: [
                                     ...
                                     <?php
                                     $sql = "select municipio , count(*) as solicitudes from detalleinfotutor group by estado order by solicitudes";
                                     $result = mysqli_query($conexion,$sql);
                                     while($registros = mysqli_fetch_array($result))
                                    {
                                    ?>
                                    '<?php echo $registros["municipio"] ?>',
                                   <?php
                                    }
                                   ?>
					...
					series: [{
				                ...
				                data: [
                                      <?php
                                     $sql = "select municipio , count(*) as solicitudes from detalleinfotutor group by municipio order by solicitudes";
                                     $result = mysqli_query($conexion,$sql);
                                     while($registros = mysqli_fetch_array($result))
                                    {
                                    ?>
